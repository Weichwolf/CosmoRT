...
#define MSS           1460
#define CUBIC_C_1024  410
#define CUBIC_BETA    717
#define CUBIC_ONE_MINUS_BETA  307
#define RCV_WSCALE    7
...
static void send_tcp_opts(net_tcp_t *c, uint8_t flags,
    ...
    if (c->ecn_enabled && !(flags & 0x02)) {
        if (c->ecn_ce_pending)   flags |= 0x40;
...
static void send_syn(net_tcp_t *c, uint8_t flags) {
    ...
    put32(opts + olen, 0); olen += 4;

    if (flags == 0x02)
        flags |= 0xC0;
    else if (flags == 0x12 && c->ecn_enabled)
        flags |= 0x40;
...
static uint32_t cubic_w(uint64_t t_ms, uint32_t w_max, uint32_t k_ms) {
    int64_t dt = (int64_t)t_ms - (int64_t)k_ms;
    int neg = 0;
    if (dt < 0) { neg = 1; dt = -dt; }
