 ...
 * already adjusted for time_namespace and CLOCK_REALTIME epoch by callers).
 * On interrupt returns -EINTR after writing remaining time to *out_left_ns.
 * On full sleep returns 0.
 *
 * Linux hrtimer_nanosleep arms a single hrtimer at the absolute deadline
 * and blocks; we delegate to sleep_interruptible_ns which does the same
 * via schedule_timeout_interruptible — the inner hrtimer is armed at
 * (now + remaining) every iteration so spurious wakes don't lose precision. */
static long sleep_until_ns(uint64_t deadline_ns, uint64_t *out_left_ns) {
    while (hrtimer_now_ns() < deadline_ns) {
        ...
        (void)sleep_interruptible_ns(deadline_ns - now);
    }
 ...
 * epoch shift. Stores nonzero in *expired when the deadline is in the past. */
static uint64_t abs_user_ns_to_kernel_ns(int clk_id, int64_t user_ns, int *expired) {
    ...
    if (clk_id == CLOCK_REALTIME || clk_id == CLOCK_REALTIME_COARSE) {
        extern int64_t rtc_epoch_sec;
        int64_t epoch_ns = (int64_t)rtc_epoch_sec * NSEC_PER_SEC;
